...
const BuyerAccountSettings = () => {
  ...
  const handleRequestEmailOtp = async () => {
    ...
    try {
      ...
      setEmailOtpMessage("OTP sent to new email. Enter the OTP to confirm.");
      setEmailStep(3);
      ...
      if (err.name === 'AbortError' || err.code === 'ECONNABORTED') {
        ...
        setEmailOtpMessage("");
        addToast(err.response?.data?.message || "Failed to send OTP. Please try again.", "error");
      }
    } finally {
      setEmailLoading(false);
    }
        ...
        <main style={{ ...styles.card, ...styles.main }}>
          {activeTab === "notifications" && (
            <>
              ...
              <ToggleRow icon={<Bell />} title="Order Alerts" desc="Receive alerts for new and updated orders" />
              <ToggleRow icon={<Eye />} title="Delivery Updates" desc="Get notified about shipment status" />
              <ToggleRow icon={<Mail />} title="Marketing Emails" desc="Receive promotional emails and newsletters" />
            </>
          )}
          ...
          {activeTab === "security" && (
            <>
              ...
              <SecurityCard title="Change Password" step={passStep}>
                {passStep === 1 && (
                  <PrimaryButton loading={passwordLoading} text="Send Password OTP" onClick={handleRequestPassOtp} />
                )}
                {passStep === 2 && (
                  <>
                    <Input value={passOtp} onChange={setPassOtp} placeholder="Enter OTP" />
                    <PrimaryButton loading={passwordLoading} text="Verify Password OTP" onClick={handleVerifyPassOtp} />
                  </>
                )}
                {passStep === 3 && (
                  <>
              ...
              <SecurityCard title="Change Email" step={emailStep}>
                ...
                {emailStep === 2 && (
                  <>
                    ...
                    <PrimaryButton loading={emailLoading} text="Send Email OTP" onClick={handleRequestEmailOtp} />
                  </>
                )}
                {emailStep === 3 && (
                  <>
                    <p style={{ margin: 0, color: '#0f172a', fontSize: 13 }}>{emailOtpMessage}</p>
...
const PrimaryButton = ({ text, loading, disabled, onClick }) => (
  <button
    onClick={onClick}
