...
import {
  Shield, Database, Loader, CheckCircle, Sprout, Cloud, Globe
} from "lucide-react";
      ...
      <div style={styles.container}>
        ...
        <main style={{ ...styles.card, ...styles.main }}>
          {activeTab === "security" && (
            ...
            <h2 style={{...styles.heading, marginTop: 32}}>Security Implemented</h2>
            <div style={styles.sectionCard}>
                <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: '8px 0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 14, color: '#334155' }}>
                        <CheckCircle size={20} color="#10b981" /> 
                        <span><strong>OTP for Sign Up:</strong> Email verification required for all new registrations.</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 14, color: '#334155' }}>
                        <CheckCircle size={20} color="#10b981" /> 
                        <span><strong>Strong Password:</strong> Enforced hashing and validation policies for data protection.</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 14, color: '#334155' }}>
                        <CheckCircle size={20} color="#10b981" /> 
                        <span><strong>Email Notification:</strong> Security alerts sent for login activity and account changes.</span>
                    </div>
                </div>
            </div>
            </>
          ...
          {activeTab === "datasources" && (
            <>
              <h2 style={styles.heading}>External Data Sources</h2>
              <div style={styles.sectionCard}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                    <div style={{ minWidth: 48, height: 48, borderRadius: 12, background: '#e0f2fe', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0284c7' }}>
                        <Database size={24} />
                    </div>
                    <div>
                        <div style={{ fontWeight: 600, color: '#0f172a', fontSize: 16 }}>data.gov.in (Mandi Prices)</div>
                        <div style={{ fontSize: 14, color: '#64748b', marginTop: 4 }}>Government open data for crop prices (example/demo API key used). Provides daily market rates for various commodities.</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                    <div style={{ minWidth: 48, height: 48, borderRadius: 12, background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#16a34a' }}>
                        <Sprout size={24} />
                    </div>
                    <div>
                        <div style={{ fontWeight: 600, color: '#0f172a', fontSize: 16 }}>Kaegro (Soil Data)</div>
                        <div style={{ fontSize: 14, color: '#64748b', marginTop: 4 }}>Location-based soil info (no API key required). Used for crop recommendations based on soil composition.</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                    <div style={{ minWidth: 48, height: 48, borderRadius: 12, background: '#fff7ed', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ea580c' }}>
                        <Cloud size={24} />
                    </div>
                    <div>
                        <div style={{ fontWeight: 600, color: '#0f172a', fontSize: 16 }}>Open-Meteo (Weather)</div>
                        <div style={{ fontSize: 14, color: '#64748b', marginTop: 4 }}>Real-time & forecast weather (free, no authentication). Provides temperature, rainfall, and humidity data.</div>
                    </div>
                  </div>
                </div>
              </div>
...
const PrimaryButton = ({ text, loading, onClick, icon }) => (
  <button
    onClick={onClick}
    disabled={loading}
    style={{
